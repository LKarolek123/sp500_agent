...
def simulate_trades(df: pd.DataFrame,
    ...
    """Minimal simulator wrapper used to validate the import and run small sweeps.

    This minimal version keeps behavior simple: it assumes a column `signal` and
    executes next-bar open entries with unit qty (for quick sanity checks).
    The full-featured simulator is available in repo history; this minimal
    implementation is temporary to unblock the expanded sweep run.
    """
    df = df.copy()
    df = df.sort_index()
    ...
    n = len(df)
    for i in range(n - 1):
        sig = df.iloc[i].get(signal_col, 0)
        ...
        exit_price = df.iloc[entry_idx]['Close']
        qty = 1
        pl = (exit_price - entry_price) * qty * (1 if sig == 1 else -1)
        equity += pl
        trades.append({'entry_index': df.index[entry_idx], 'exit_index': df.index[entry_idx], 'direction': sig,
                       'entry_price': entry_price, 'exit_price': exit_price, 'qty': qty, 'pl': pl})
        equity_curve.append(equity)
